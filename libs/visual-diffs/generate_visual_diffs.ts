...
import { summarizePRChanges } from "./summarize_changes.js";
...
export async function generate_visual_diffs_for_pr(
  ...
  core.info(
    `Looking for link files matching type patterns (e.g., *.doc.gdrive.json, *.pdf.gdrive.json)`
  );
  ...
  const currentBranch = await execute_git(
    "rev-parse",
    ["--abbrev-ref", "HEAD"],
    { silent: true }
  );
  core.info(`Current branch: ${currentBranch.stdout.trim()}`);
  const currentHead = await execute_git("rev-parse", ["HEAD"], {
    silent: true,
  });
  core.info(`Current HEAD SHA: ${currentHead.stdout.trim()}`);
  const branchStatus = await execute_git("status", ["--short"], {
    silent: true,
  });
  ...
  try {
    core.info("Generating PR summary using Gemini...");
    await summarizePRChanges(
      params.octokit,
      params.owner,
      params.repo,
      params.pr_number
    ...
    core.warning(`Failed to generate PR summary: ${error.message}`);
    // Continue with visual diff generation even if summary fails
  }
  ...
  try {
    ...
    core.error(
      `Failed to prepare output directory ${params.output_base_dir}: ${
  ...
  const link_file_regex_vd = new RegExp(
    `\\.(${known_extensions_vd})\\.gdrive\\.json$`
  );
  core.debug(`Using regex to find link files: ${link_file_regex_vd}`);

  try {
    const files_iterator = params.octokit.paginate.iterator(
    ...
    for await (const { data: files } of files_iterator) {
      for (const file of files) {
        ...
        if (
          ...
          core.info(
            ` -> Found candidate: ${file.filename} (Status: ${file.status}) -> Base Name: ${base_name} (Type: ${file_type})`
          );
          changed_link_files.push({
            path: file.filename,
            base_name,
            file_type,
          });
          ...
          core.debug(
            ` -> Skipping file: ${file.filename} (Status: ${
              file.status
            }, Pattern mismatch: ${!match})`
          );
        }
      }
    }
    core.info(
      `Found ${changed_link_files.length} added/modified/renamed link file(s) matching pattern to process.`
    );
  } catch (error: any) {
  ...
  if (changed_link_files.length === 0) {
    core.info(
      "No relevant changed link files found in this PR update. Nothing to generate."
    );
  ...
  try {
    temp_dir = await fs.promises.mkdtemp(
      path.join(os.tmpdir(), `visual-diff-${params.pr_number}-`)
    );
    core.info(`Using temporary directory: ${temp_dir}`);
  } catch (tempError) {
    core.error(
      `Failed to create temporary directory: ${(tempError as Error).message}`
    );
  ...
  for (const link_file of changed_link_files) {
    ...
    let original_name: string; // Base name without extension
    let folder_name: string; // Name used for output folder (with extension)

    // 1. Get File ID and MIME Type from link file content (using head_sha)
    try {
      core.debug(
        `Fetching content for: ${link_file.path} at ref ${params.head_sha}`
      );
      const { data: content_response } =
      ...
      if (
        ...
        if (
          ...
          original_name =
            typeof file_data.name === "string" && file_data.name.trim()
              ? file_data.name.trim()
              : link_file.base_name;
          // Remove any known extension from original_name to avoid duplication
          const extension_regex = new RegExp(
            `\\.(?:${Object.values(MIME_TYPE_TO_EXTENSION).join("|")})$`,
            "i"
          );
          original_name = original_name.replace(extension_regex, "");
          // Get the extension from MIME_TYPE_TO_EXTENSION
          const extension =
            MIME_TYPE_TO_EXTENSION[mime_type] || link_file.file_type;
          // Construct folder_name by appending the MIME type's extension
          folder_name = `${original_name}.${extension}`;
          core.info(
            `   - Extracted Drive ID: ${file_id}, MIME Type: ${mime_type}, Original Name: ${original_name}, Folder Name: ${folder_name}`
          );
        } else {
          core.warning(
            `   - Could not find 'id' and 'mimeType' (both strings) in JSON content of ${link_file.path}. Skipping.`
          );
          continue;
        }
      } else {
        core.warning(
          `   - Could not retrieve valid base64 content for ${link_file.path} (SHA: ${params.head_sha}). Skipping.`
        );
        continue;
      }
    } catch (error: any) {
      if (error.status === 404) {
        core.warning(
          `   - Link file ${link_file.path} not found at ref ${params.head_sha}. It might have been moved or deleted. Skipping.`
        );
      } else {
        core.warning(
          `   - Failed to get or parse content of ${link_file.path} at ref ${params.head_sha}: ${error.message}. Skipping.`
        );
      }
      continue;
    }
    ...
    if (!file_id || !mime_type || !original_name || !folder_name) {
      core.error(
        `Logic error: file_id, mime_type, original_name, or folder_name missing after successful parse for ${link_file.path}`
      );
      continue;
    }
    ...
    const sanitized_base_name = folder_name
    ...
    if (!fetch_success) {
      core.warning(
        `   - Failed to fetch PDF for ${link_file.path} (Drive ID: ${file_id}). Skipping PNG generation for this file.`
      );
      continue;
    }
    ...
    core.info(
      `   - Converting PDF to PNGs in directory: ${image_output_dir_absolute_path} (relative: ${image_output_dir_relative_path})`
    );
    // Clean the specific output directory to avoid stale files
    await fs.promises.rm(image_output_dir_absolute_path, {
      recursive: true,
      force: true,
    });
    ...
    const generated_pngs = await convert_pdf_to_pngs(
      temp_pdf_path,
    ...
    if (generated_pngs.length > 0) {
      ...
      processed_files_info.push(
        `'${link_file.path}' (${generated_pngs.length} pages)`
      );
      core.info(
        `   - Generated ${generated_pngs.length} PNGs for ${link_file.path}`
      );
    } else {
      core.warning(
        `   - No PNGs generated from PDF for ${link_file.path}. Conversion might have failed.`
      );
    }
  ...
  if (temp_dir) {
    ...
    await fs.promises
      ...
      .catch((rmErr) =>
        core.warning(
          `Failed to remove base temp directory ${temp_dir}: ${rmErr.message}`
        )
      );
  }
  ...
  if (total_pngs_generated > 0) {
    const commit_message = `${SKIP_CI_TAG} Generate visual diff PNGs for PR #${
      params.pr_number
    ...
    try {
      ...
      const postCommitBranch = await execute_git(
        "rev-parse",
        ["--abbrev-ref", "HEAD"],
        { silent: true }
      );
      core.info(`Post-commit branch: ${postCommitBranch.stdout.trim()}`);
      const postCommitHead = await execute_git("rev-parse", ["HEAD"], {
        silent: true,
      });
      core.info(`Post-commit HEAD SHA: ${postCommitHead.stdout.trim()}`);
      const postCommitLog = await execute_git("log", ["-1", "--pretty=%H %s"], {
        silent: true,
      });
      core.info(`Latest commit:\n${postCommitLog.stdout}`);
    } catch (commitError) {
      core.error(
        "Visual diff generation succeeded, but committing/pushing PNGs failed."
      );
      throw commitError;
    }
  } else {
