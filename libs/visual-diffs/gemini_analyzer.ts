 ...
 */
export async function analyze_image_diff_with_gemini(
  base_png_path: string | null,
  head_png_path: string | null,
  api_key: string,
  model_name: string = "gemini-1.5-flash-latest" // Default model
): Promise<string | null> {
  if (!api_key) {
  ...
  if (!base_png_path && !head_png_path) {
    core.warning(
      "Both base and head image paths are null. Cannot perform analysis."
    );
    return null;
  }
  ...
  try {
    ...
    if (response.promptFeedback?.blockReason) {
      core.error(
        `Gemini request blocked: ${response.promptFeedback.blockReason}`
      );
      if (response.promptFeedback.safetyRatings) {
        core.error(
          `Safety Ratings: ${JSON.stringify(
    ...
    core.info(
      `Gemini analysis received: ${analysis_text.substring(0, 100)}...`
    ); // Log first 100 chars
 ...
 */
export function format_pr_comment(
  analysis_results: {
    filePath: string;
    page: number;
  ...
  const results_by_file: {
    [filePath: string]: { page: number; analysis: string | null }[];
  } = {};
  ...
  for (const filePath in results_by_file) {
    ...
    file_results.sort((a, b) => {
      if (a.page === 0 && b.page !== 0) return -1; // File status comes first
    ...
    for (const result of file_results) {
      if (result.page === 0) {
        ...
        comment_body += `  - **ページ ${result.page}:** ${
          result.analysis || "*分析結果なし*"
        }\n`;
      }
    }
