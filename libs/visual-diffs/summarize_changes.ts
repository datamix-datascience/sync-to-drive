...
function initializeGeminiAPI(): GoogleGenAI {
  const apiKey = core.getInput("gemini_api_key", { required: true });
  if (!apiKey) {
    throw new Error(
      "Gemini API key is missing. Please configure the gemini_api_key input."
    );
  }
...
async function getDiffImages(
  files: any[]
): Promise<{ inlineData: { mimeType: string; data: string } }[]> {
  const diffImages = files
    .filter(
      (file) =>
        file.filename.startsWith("_diff_") && file.filename.endsWith(".png")
    )
    .map(async (file) => {
...
async function postSummaryToPR(
  octokit: Octokit,
  owner: string,
  repo: string,
  pr_number: number,
...
export async function summarizePRChanges(
  octokit: Octokit,
  owner: string,
  repo: string,
  pr_number: number
