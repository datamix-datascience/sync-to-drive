...
import { Octokit } from "@octokit/rest"; // Only need the type here

// Use Octokit type from the main library installation
type OctokitInstance = InstanceType<typeof Octokit>;

// Create PR with Retry - Handles existing PRs
export async function create_pull_request_with_retry(
  octokit: OctokitInstance, // Pass the initialized Octokit instance
  ...
  try {
    ...
    core.warning(`Could not fetch repository info to confirm default branch. Using provided base '${params.base}'. Error: ${(repoError as Error).message}`);
  ...
  for (let attempt = 0; attempt < max_retries; attempt++) {
    try {
      ...
      if ((http_error?.status === 422 || http_error?.status === 404 || http_error?.status === 403) && attempt < max_retries - 1) {
        ...
        if (http_error?.message) core.error(`Final Error: ${http_error.message}`);
