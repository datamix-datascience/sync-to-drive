 ...
 */
export async function summarizeImageDiffEnglish(
  url1: string,
  url2: string
): Promise<string> {
 ...
 */
export async function summarizeImageDiffJapanese(
  url1: string,
  url2: string
): Promise<string> {
  // Initialize the Gemini client; ensure GEMINI_API_KEY env var is set
 ...
 */
export async function summarizeNewImageEnglish(url: string): Promise<string> {
  // Initialize the Gemini client; ensure GEMINI_API_KEY env var is set
  const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY! });
 ...
 */
export async function summarizeNewImageJapanese(url: string): Promise<string> {
  // Initialize the Gemini client; ensure GEMINI_API_KEY env var is set
  const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY! });
 ...
 */
export async function generatePRCommentsSeparated(
  owner: string,
  repo: string,
  prNumber: number,
  diffDir: string = "_diff_"
  ...
  let englishCommentParts: string[] = ["# Visual Differences Summary\n"];
  let japaneseCommentParts: string[] = ["# 視覚的な差分の要約\n"];
    ...
    for (const file of files) {
      try {
        ...
        if (urls && (urls.before || urls.after)) {
          englishCommentParts.push(
            `### Page ${slidePage.replace(".png", "")}\n`
          );
          japaneseCommentParts.push(
            `### ページ ${slidePage.replace(".png", "")}\n`
          );
          ...
          if (encodedBefore && encodedAfter) {
            ...
            try {
              const diffSummaryEnglish = await summarizeImageDiffEnglish(
                encodedBefore,
                encodedAfter
              );
              const diffSummaryJapanese = await summarizeImageDiffJapanese(
                encodedBefore,
                encodedAfter
              );
              ...
              japaneseCommentParts.push(
                `エラーのため比較を生成できませんでした: ${compareError.message}\n`
              );
            }
            ...
            try {
              ...
              const contentSummaryEnglish = await summarizeNewImageEnglish(
                encodedAfter
              );
              const contentSummaryJapanese = await summarizeNewImageJapanese(
                encodedAfter
              );
              ...
              japaneseCommentParts.push(
                `エラーのため内容の要約を生成できませんでした: ${contentError.message}\n`
              );
            }
        ...
        japaneseCommentParts.push(
          `エラーのためこのページを処理できませんでした: ${fileError.message}\n`
        );
      }
    }
  }
 ...
 */
export async function postSeparatedPRComments(
  owner: string,
  repo: string,
  prNumber: number,
  diffDir: string = "_diff_"
): Promise<void> {
  const comments = await generatePRCommentsSeparated(
    owner,
    repo,
    prNumber,
    diffDir
