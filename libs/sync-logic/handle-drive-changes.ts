...
export async function handle_drive_changes(
  ...
  try {
    ...
    const expected_local_files = new Map<string, { type: 'link' | 'content' | 'svg', driveItem: DriveItem }>(); // Added 'svg' type
    ...
    for (const { path: drive_path, item: drive_item } of drive_files_with_paths) {
      ...
      if (is_google_slide) {
        ...
        if (expected_svg_info && expected_svg_info.driveItem.id === drive_item.id) {
            ...
            if (!local_svg_info) {
                needs_update = true; reason = "SVG file missing locally";
            } else {
                ...
                if (corresponding_link_expected?.type === 'link' && corresponding_link_expected.driveItem.id === drive_item.id) {
                    ...
                    if (!local_link_info) {
                        ...
                        try {
                            ...
                            const link_data = JSON.parse(link_content);
                            if (link_data.modifiedTime !== drive_item.modifiedTime) {
                                needs_update = true; reason = `SVG update triggered by ModifiedTime mismatch in link file`;
                            }
