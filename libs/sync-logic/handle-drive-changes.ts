...
import {
  DriveFileWithPath,
  list_drive_files_recursively,
} from "../google-drive/list.js";
...
import {
  ...
  LINK_FILE_MIME_TYPES,
  construct_link_file_name,
} from "../google-drive/file_types.js";
...
import {
  generate_slide_svg,
  write_svg_file,
} from "../google-slides/slides_to_svg.js";
...
function get_repo_info(): { owner: string; repo: string } {
  ...
  if (!owner || !repo) {
    throw new Error(
      `Could not parse owner and repo from GITHUB_REPOSITORY: ${repo_full_name}`,
    );
  }
  return { owner, repo };
}
...
async function determineInitialBranch(repo_info: {
  ...
  try {
    ...
    const current_branch_result = await execute_git(
      "rev-parse",
      ["--abbrev-ref", "HEAD"],
      { silent: true },
    );
    ...
    if (parsedBranch && parsedBranch !== "HEAD") {
      ...
      core.info(
        `Determined initial branch from git rev-parse: '${branchName}'`,
      );
      return branchName;
    }
    ...
    core.error(
      `Failed to determine initial branch name: ${(error as Error).message}`,
    );
    // This is critical, cannot reliably proceed or clean up without the initial branch.
    throw new Error(
      "Could not determine the initial branch. Action cannot continue.",
    );
  }
}
...
export async function handle_drive_changes(
  ...
  visual_diff_output_dir: string,
): Promise<HandleDriveChangesResult> {
  core.info(
    `Handling potential incoming changes from Drive folder: ${folder_id} for trigger event: ${trigger_event_name}`,
  );
  ...
  if (trigger_event_name === "push") {
    ...
    core.info(
      "Skipping Drive-to-Git synchronization logic in 'handle_drive_changes'.",
    );
  ...
  core.info(
    `Proceeding with Drive-to-Git synchronization for event '${trigger_event_name}'.`,
  );
  ...
  try {
    ...
    core.info(
      `Initial branch is '${initial_branch}'. Creating temporary state branch '${original_state_branch}'`,
    );
    const initial_commit_hash = (
      await execute_git("rev-parse", ["HEAD"], { silent: true })
    ).stdout.trim();
    if (!initial_commit_hash)
      throw new Error("Could not get initial commit hash.");
    await execute_git("checkout", [
      "-b",
      original_state_branch,
      initial_commit_hash,
    ]);
    ...
    const initial_local_map = new Map(
      initial_local_files_list.map((f) => [
        f.relative_path.replace(/\\/g, "/"),
        f,
      ]),
    );
    core.info(
      `Found ${initial_local_map.size} relevant local files in original state.`,
    );
    ...
    try {
      ...
      core.info(
        `Found ${drive_files_with_paths.length} files and ${drive_folders.size} folders in Drive.`,
      );
    } catch (error) {
      core.error(
        `Failed list Drive content for folder ${folder_id}: ${(error as Error).message}. Aborting incoming sync logic.`,
      );
      return result;
    }
    ...
    for (const {
      path: drive_path,
      item: drive_item,
    } of drive_files_with_paths) {
      // <-- Iterate the array
      if (!drive_item.id || !drive_item.name || !drive_item.mimeType) {
        core.warning(
          `Skipping Drive item with missing id, name, or mimeType. Path: '${drive_path}', ID: ${drive_item.id || "N/A"}`,
        );
        continue;
      }
      ...
      const is_google_slide =
        drive_item.mimeType === "application/vnd.google-apps.presentation";
      const needs_link_file = LINK_FILE_MIME_TYPES.includes(
        drive_item.mimeType,
      );
      ...
      if (needs_link_file) {
        ...
        const link_filename = construct_link_file_name(
          base_name,
          drive_item.id,
          drive_item.mimeType,
        );
        ...
        expected_link_path =
          drive_dir === "."
            ? link_filename
            : path.join(drive_dir, link_filename).replace(/\\/g, "/");
      }
      ...
      if (is_google_slide && expected_link_path) {
        // Derive SVG path from link path, then prepend visual_diff_output_dir directory
        const relative_svg_path = expected_link_path.replace(
          /\.gdrive\.json$/i,
          ".export.svg",
        );
        expected_svg_path = path
          .join(visual_diff_output_dir, relative_svg_path)
          .replace(/\\/g, "/");
      }

      // Add expected *local* files to the map
      if (!is_google_doc) {
        // Content file expected for non-Google Docs
        if (expected_local_files.has(expected_content_path)) {
          // This should be rare now unless non-link files somehow clash exactly
          core.debug(
            `Duplicate content path mapping detected: ${expected_content_path}. Overwriting with Drive ID ${drive_item.id}.`,
          );
        }
        expected_local_files.set(expected_content_path, {
          type: "content",
          driveItem: drive_item,
        });
      }
      if (expected_link_path) {
        // Link file expected for Google Docs and PDFs (and Slides)
        if (expected_local_files.has(expected_link_path)) {
          ...
          core.error(
            `FATAL: Duplicate link path mapping detected even with file ID in name: ${expected_link_path}. Existing ID: ${expected_local_files.get(expected_link_path)?.driveItem.id}, New ID: ${drive_item.id}. This indicates a logic error.`,
          );
          // Consider throwing an error or handling this unexpected state
        }
        expected_local_files.set(expected_link_path, {
          type: "link",
          driveItem: drive_item,
        });
      }
      if (expected_svg_path) {
        // SVG file expected for Google Slides
        if (expected_local_files.has(expected_svg_path)) {
          // Also should not happen due to unique ID in name derivation
          core.error(
            `FATAL: Duplicate SVG path mapping detected: ${expected_svg_path}. Existing ID: ${expected_local_files.get(expected_svg_path)?.driveItem.id}, New ID: ${drive_item.id}.`,
          );
        }
        expected_local_files.set(expected_svg_path, {
          type: "svg",
          driveItem: drive_item,
        });
      }
    ...
    core.info(
      `Calculated ${expected_local_files.size} expected local files (content, links, svgs) based on Drive state.`,
    );
    ...
    for (const {
      ...
      if (!is_google_doc) {
        ...
        if (
          ...
          if (!local_file_info) {
            needs_update = true;
            reason = `Content file missing locally`;
          } else {
            if (
              expected_content_info.driveItem.hash &&
              local_file_info.hash !== expected_content_info.driveItem.hash
            ) {
              needs_update = true;
              ...
              const expected_link_path_for_content =
                drive_dir === "."
                  ? construct_link_file_name(
                      base_name,
                      drive_item.id,
              ...
              const corresponding_link_expected = expected_local_files.get(
                expected_link_path_for_content,
              );
              if (
                corresponding_link_expected?.type === "link" &&
                corresponding_link_expected.driveItem.id === drive_item.id
              ) {
                const local_link_info = initial_local_map.get(
                  expected_link_path_for_content,
                );
                if (!local_link_info) {
                  needs_update = true;
                  reason =
                    "Corresponding link file missing locally for timestamp check";
                } else {
                  try {
                    const link_content = await fs_promises.readFile(
                      local_link_info.path,
                      "utf-8",
                    );
                    const link_data = JSON.parse(link_content);
                    if (link_data.modifiedTime !== drive_item.modifiedTime) {
                      needs_update = true;
                      reason = `ModifiedTime mismatch via link file`;
                    }
                  } catch {
                    needs_update = true;
                    reason = `Cannot read link file for timestamp check`;
                  }
                }
              } else {
                core.debug(
                  `No Drive hash and no corresponding link file found for ${expected_content_path}. Cannot reliably check for update.`,
                );
              }
            }
          }
          if (needs_update) {
            ...
            update_reasons.push(
              `Content file '${expected_content_path}': ${reason}`,
            );
          }
        }
      }
      ...
      if (needs_link_file) {
        ...
        if (
          expected_link_info &&
          expected_link_info.driveItem.id === drive_item.id
        ) {
          ...
          if (!local_file_info) {
            needs_update = true;
            reason = `Link file missing locally`;
          } else {
            try {
              const link_content = await fs_promises.readFile(
              ...
              const link_data = JSON.parse(link_content);
              if (
                link_data.id !== drive_item.id ||
                link_data.name !== drive_item.name || // Check name consistency
                link_data.modifiedTime !== drive_item.modifiedTime
              ) {
                needs_update = true;
                reason = `Link file data mismatch (ID, Name, or modifiedTime)`;
              }
            } catch (error) {
              needs_update = true;
              reason = `Cannot read/parse local link file`;
            }
          }
          if (needs_update) {
            item_needs_update = true;
      ...
      if (is_google_slide) {
        ...
        const relative_svg_filename = actual_link_file_path.replace(
          /\.gdrive\.json$/i,
          ".export.svg",
        );
        const expected_svg_path = path
          .join(visual_diff_output_dir, relative_svg_filename)
          .replace(/\\/g, "/");
        ...
        if (
          expected_svg_info &&
          expected_svg_info.driveItem.id === drive_item.id
        ) {
          ...
          if (!local_svg_info) {
            ...
            if (
              ...
              core.debug(
                `Could not find corresponding link file (${actual_link_file_path}) to check modifiedTime for SVG (${expected_svg_path}). Assuming update needed if link changed.`,
              );
              // If the link file itself needed an update (checked in block B), that's enough reason to update the SVG too.
              if (
                update_reasons.some((r) =>
                  r.startsWith(`Link file '${actual_link_file_path}'`),
                )
              ) {
                needs_update = true;
      ...
      if (item_needs_update) {
        core.info(
          ` -> Change detected for Drive item: ${drive_item.name} (ID: ${drive_item.id}, Path: ${drive_path}). Reasons: ${update_reasons.join("; ")}`,
        );
    ...
    expected_local_files.forEach((_, expected_path) => {
      // Use the expected_local_files map
    ...
    for (const initial_dir of initial_local_dirs) {
      if (!expected_local_dirs.has(initial_dir)) {
        ...
        if (!parent_deleted && !drive_folder_exists) {
          core.info(
            ` -> Deletion identified for potentially empty local directory: ${initial_dir}`,
          );
    ...
    core.info(
      `Identified ${drive_items_needing_processing.size} Drive items needing add/update processing.`,
    );
    core.info(
      `Identified ${local_paths_identified_for_deletion.size} local paths/folders for deletion.`,
    );
    core.endGroup();
    ...
    if (local_paths_identified_for_deletion.size > 0) {
      ...
      const sorted_deletion_paths = Array.from(
        local_paths_identified_for_deletion,
      ).sort(
        (a, b) =>
      ...
      for (const local_path_to_delete of sorted_deletion_paths) {
        try {
          const stats = await fs_promises
            .lstat(local_path_to_delete)
            .catch(() => null);
          if (stats) {
            core.info(`   - Removing: ${local_path_to_delete}`);
            await fs_promises.rm(local_path_to_delete, {
              recursive: true,
              force: true,
            });
            ...
            core.debug(
              `   - Skipping removal, path already gone: ${local_path_to_delete}`,
            );
          }
        } catch (error) {
          core.error(
            `   - Failed to remove ${local_path_to_delete}: ${(error as Error).message}`,
          );
        }
      }
    }
    ...
    if (drive_items_needing_processing.size > 0) {
      core.info(
        `Applying ${drive_items_needing_processing.size} additions/updates...`,
      );
      for (const {
        driveItem,
        targetContentPath,
      } of drive_items_needing_processing.values()) {
        core.info(
          `   - Processing Drive item: ${driveItem.name || `(ID: ${driveItem.id})`} -> Target local content path: ${targetContentPath}`,
        );
        try {
          ...
          if (
            driveItem.mimeType === "application/vnd.google-apps.presentation"
          ) {
            core.info(
              `   -> Item is Google Slides. Attempting SVG generation...`,
            );
            // 1. Fetch Slides JSON content using the specific function and auth client
            const presentation_json = await fetch_google_slide_json(
              driveAuthClient,
              driveItem.id,
            );
            if (presentation_json) {
              ...
              if (svg_string) {
                ...
                const link_file_base_name = construct_link_file_name(
                  base_name,
                  driveItem.id,
                  driveItem.mimeType,
                ); // "MyPresentation_id.gslides.gdrive.json"
                const svg_file_base_name = link_file_base_name.replace(
                  /\.gdrive\.json$/i,
                  ".export.svg",
                ); // "MyPresentation_id.gslides.export.svg"
                ...
                // Construct the final path within the visual_diff_output_dir folder
                // If relative_item_dir is ".", path.join will correctly place svg_file_base_name directly under visual_diff_output_dir
                const output_svg_path = path
                  .join(
                ...
                const write_success = await write_svg_file(
                  svg_string,
                  output_svg_path,
                );
                if (write_success) {
                  core.info(
                    `   -> Successfully generated and saved SVG for ${driveItem.name} at ${output_svg_path}`,
                  );
          ...
          core.error(
            `   - Failed to process item from Drive ${driveItem.name || `(ID: ${driveItem.id})`} to ${targetContentPath}: ${(error as Error).message}`,
          );
        }
      }
    }
    ...
    if (!changes_applied) {
      core.info(
        "No file system changes were applied. Checking Git status anyway.",
      );
    }
    ...
    await execute_git("ls", ["-laR"], {
      silent: false,
      ignoreReturnCode: true,
    }); // Use 'dir /s /b' on Windows if needed
    // *** END ADDED DEBUGGING ***
    ...
    if (added_updated_display.length > 0)
      commit_detail_lines.push(
        `- Add/Update (from Drive): ${added_updated_display.map((p) => `'${p}'`).join(", ")}`,
      );

    const removed_display_paths = Array.from(
      local_paths_identified_for_deletion,
    ).sort((a, b) => a.localeCompare(b));
    if (removed_display_paths.length > 0)
      commit_detail_lines.push(
        `- Remove (local paths): ${removed_display_paths.map((p) => `'${p}'`).join(", ")}`,
      );
    ...
    const commit_message = [
      ...
      ...(commit_detail_lines.length > 0
        ? commit_detail_lines
        : ["- No file content changes detected."]), // Ensure commit body isn't empty if only deletions happened
    ...
    try {
      ...
      const sync_commit_hash = (
        await execute_git("rev-parse", ["HEAD"], { silent: true })
      ).stdout.trim(); // Get HEAD SHA of the new commit
      core.info(
        `Created sync commit ${sync_commit_hash} on temporary branch '${original_state_branch}'.`,
      );
      ...
      const local_branch_exists_check = await execute_git(
        "show-ref",
        ["--verify", `refs/heads/${head_branch}`],
        { ignoreReturnCode: true, silent: true },
      );
      const remote_branch_exists_check = await execute_git(
        "ls-remote",
        ["--exit-code", "--heads", "origin", head_branch],
        { ignoreReturnCode: true, silent: true },
      );
      ...
      if (local_branch_exists_check.exitCode === 0) {
        core.info(
          `Branch ${head_branch} exists locally. Checking it out and resetting...`,
        );
        ...
        core.info(
          `Branch ${head_branch} exists remotely. Fetching, checking out, and resetting...`,
        );
        try {
          await execute_git("fetch", [
            "origin",
          ...
          core.warning(
            `Failed to fetch/checkout/reset remote branch ${head_branch}. Will create new local branch. Error: ${(fetchCheckoutError as Error).message}`,
          );
      ...
      if (branch_creation_needed) {
        core.info(
          `Branch ${head_branch} does not exist or couldn't be reset. Creating it from commit ${sync_commit_hash}...`,
        );
      ...
      core.info(
        `Attempting to create or update Pull Request: ${pr_title} (${head_branch} -> ${initial_branch})`,
      );
      const pr_result = await create_pull_request_with_retry(
        octokit,
        pr_params,
      );
      if (pr_result) {
      ...
      core.error(
        `Failed during commit, push, or PR creation: ${(error as Error).message}`,
      );
      if (!result.head_branch) result = {};
    } finally {
      core.endGroup();
    ...
    core.error(
      `Error during Drive change handling for folder ${folder_id}: ${(error as Error).message}`,
    );
    result = {};
  } finally {
    core.startGroup(`Cleaning up Git State`);
    core.info(
      `Cleaning up temporary branch '${original_state_branch}' and returning to '${initial_branch}'`,
    );
    try {
      const current_cleanup_branch_result = await execute_git(
        "rev-parse",
        ["--abbrev-ref", "HEAD"],
        { silent: true, ignoreReturnCode: true },
      );
      const current_cleanup_branch =
        current_cleanup_branch_result.stdout.trim();

      if (current_cleanup_branch !== initial_branch && initial_branch) {
        core.info(
          `Currently on branch '${current_cleanup_branch || "detached HEAD"}', checking out initial branch '${initial_branch}'...`,
        );
        ...
        core.warning(
          `Could not determine initial branch for cleanup checkout. Staying on '${current_cleanup_branch || "detached HEAD"}'`,
        );
      }

      if (original_state_branch) {
        const branch_check = await execute_git(
          "show-ref",
          ["--verify", `refs/heads/${original_state_branch}`],
          { ignoreReturnCode: true, silent: true },
        );
        if (branch_check.exitCode === 0) {
          core.info(
            `Deleting temporary state branch '${original_state_branch}'...`,
          );
          await execute_git("branch", ["-D", original_state_branch]);
        } else {
          core.debug(
            `Temporary state branch '${original_state_branch}' not found for deletion.`,
          );
        }
      }
    } catch (checkoutError) {
      core.warning(
        `Failed to fully clean up Git state. Manual cleanup may be needed. Error: ${(checkoutError as Error).message}`,
      );
    } finally {
