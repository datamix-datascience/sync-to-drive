 ...
 */
export async function generate_slide_svg(presentation_json) {
    ...
    try {
        ...
        core.error(`   - Failed during Slides JSON to SVG conversion: ${error.message}`);
        if (error.stack) {
            core.error(error.stack || "Unknown Error"); // Log stack for WASM errors
        }
        return null;
    }
}
 ...
 */
export async function write_svg_file(svg_string, output_svg_path) {
    try {
        ...
        core.error(`   - Failed to write SVG file to ${output_svg_path}: ${error.message}`);
        return false;
    }
}
