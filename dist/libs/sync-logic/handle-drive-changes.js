...
export async function handle_drive_changes(folder_id, on_untrack_action, trigger_event_name, git_user_name, git_user_email) {
    ...
    const drive_items_causing_update = [];
    ...
    try {
        ...
        let drive_files;
        let drive_folders; // Keep track of Drive folders too
        try {
        ...
        const drive_items_needing_processing = new Map(); // Use Drive ID as key
            ...
            if (!local_file_info) {
                ...
                if (expected_info.type === 'content') {
                    if (expected_info.driveItem.hash && local_file_info.hash !== expected_info.driveItem.hash) {
                        needs_update = true;
                        reason = `Content hash mismatch (Local: ${local_file_info.hash}, Drive: ${expected_info.driveItem.hash})`;
                    }
                        ...
                        if (corresponding_link_expected?.type === 'link') {
                            ...
                            if (!local_link_info) {
                                needs_update = true;
                                reason = "Corresponding link file missing locally, cannot verify content timestamp";
                            }
                            else {
                                try {
                                    ...
                                    if (link_data.modifiedTime !== expected_info.driveItem.modifiedTime) {
                                        needs_update = true;
                                        reason = `Inferred content update needed due to modifiedTime mismatch in corresponding link file`;
                                    }
                                }
                                catch {
                                    needs_update = true;
                                    reason = `Cannot read corresponding link file ${local_link_info.path}, cannot verify content timestamp`;
                                }
                            }
                        }
                        else {
                    ...
                    try {
                        ...
                        reason = `Cannot read/parse local link file: ${error.message}`;
                    }
                }
            }
            if (needs_update) {
        ...
        if (added_updated_display_paths.length > 0)
            commit_detail_lines.push(`- Add/Update (from Drive): ${added_updated_display_paths.map(p => `'${p}'`).join(", ")}`);
        ...
        try {
            ...
            if (local_branch_exists) {
                ...
                try {
                    ...
                    core.warning(`Failed to fetch/checkout/reset remote branch ${head_branch}. Creating new local branch from sync commit. Error: ${fetchCheckoutError.message}`);
            ...
            const pr_body = format_pr_body(folder_id, run_id, drive_items_causing_update, // Pass the list of Drive items
