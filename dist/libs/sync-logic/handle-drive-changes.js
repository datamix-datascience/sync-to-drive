...
export async function handle_drive_changes(folder_id, on_untrack_action, trigger_event_name) {
    ...
    try {
        ...
        const found_local_keys = new Set(); // Keep track of local keys that correspond to a found Drive item
            ...
            if (is_google_doc) {
                ...
                if (!local_link_info) {
                    ...
                    try {
                        ...
                        core.warning(`Failed to read/parse link file '${local_link_info.path}': ${error.message}. Marking for update.`);
                        needs_processing = true;
        ...
        try {
            ...
            const pr_body = format_pr_body(folder_id, run_id, successfully_added_updated_items, // Pass the list of successfully processed Drive items
