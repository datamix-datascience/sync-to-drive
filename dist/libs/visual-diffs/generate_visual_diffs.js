...
import { execute_git } from '../git.js'; // Assuming GitResult type exists
 ...
 */
async function stage_commit_and_push_changes(changes_dir, commit_message, git_user_email, git_user_name, target_branch // Need the branch name for the push command
) {
    ...
    try {
        ...
        const status_result = await execute_git('status', ['--porcelain', '--', changes_dir], // Limit status check to the target dir
...
export async function generate_visual_diffs_for_pr(params) {
    ...
    try {
        ...
        if (error.stderr)
            core.error(`Git stderr: ${error.stderr}`);
        core.endGroup();
    ...
    try {
        ...
        for await (const { data: files } of files_iterator) {
            for (const file of files) {
                ...
                if (link_file_regex_vd.test(path.basename(file.filename)) &&
    ...
    try {
        ...
        if (temp_dir)
            await fs.promises.rm(temp_dir, { recursive: true, force: true }).catch(() => { });
        throw tempError;
    }
