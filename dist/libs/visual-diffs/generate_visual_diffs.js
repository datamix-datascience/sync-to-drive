...
import { analyze_image_diff_with_gemini, format_pr_comment, } from "./gemini_analyzer.js";
...
async function should_skip_generation(branch_name) {
    ...
    try {
        ...
        if (latest_commit_message_result.exitCode !== 0 ||
 ...
 */
async function generate_images_for_ref(ref, link_files, image_output_root_dir, temp_dir, octokit, drive, owner, repo, resolution_dpi) {
    core.startGroup(`Generating images for ref '${ref}' into '${image_output_root_dir}'`);
    const results = [];
    ...
    for (const link_file of link_files) {
        ...
        let generated_pngs = []; // Initialize here
        ...
        try {
            ...
            if ("content" in content_response &&
                ...
                if (file_data &&
                    ...
                    core.warning(`   - Could not find 'id' and 'mimeType' (both strings) in JSON content of ${link_file.path} at ref '${ref}'. Skipping image generation.`);
                    results.push({
                ...
                core.warning(`   - Could not retrieve valid base64 content for ${link_file.path} at ref '${ref}'. Skipping.`);
                results.push({
                    link_file_path: link_file.path,
                    base_name: link_file.base_name,
                    png_paths: [],
            ...
            if (error.status === 404) {
                core.info(`   - Link file ${link_file.path} not found at ref '${ref}'. Assuming it was added in the PR. Skipping base image generation.`);
            }
            else {
                core.warning(`   - Failed to get or parse content of ${link_file.path} at ref '${ref}': ${error.message}. Skipping.`);
            }
            results.push({
                link_file_path: link_file.path,
                base_name: link_file.base_name,
        ...
        if (!file_id || !mime_type || !original_name) {
            core.error(`Logic error: file_id, mime_type, or original_name missing after successful parse for ${link_file.path} at ref '${ref}'`);
            results.push({
                link_file_path: link_file.path,
                base_name: link_file.base_name,
                png_paths: [],
        ...
        const temp_pdf_path = path.join(pdf_temp_dir, `${sanitized_base_name}_${link_file.base_name}.pdf`); // Add link file base name for uniqueness
        const fetch_success = await fetch_drive_file_as_pdf(drive, file_id, mime_type, temp_pdf_path);
        if (!fetch_success) {
            core.warning(`   - Failed to fetch PDF for ${link_file.path} (Drive ID: ${file_id}, ref: ${ref}). Skipping PNG generation.`);
            results.push({
                link_file_path: link_file.path,
                base_name: link_file.base_name,
                png_paths: [],
        ...
        const image_output_dir_absolute_path = path.join(image_output_root_dir, image_output_dir_relative_path);
        core.info(`   - Converting PDF to PNGs in directory: ${image_output_dir_absolute_path}`);
        // Ensure the output directory exists before conversion
        await fs.promises.mkdir(image_output_dir_absolute_path, {
            recursive: true,
        });
        generated_pngs = await convert_pdf_to_pngs(temp_pdf_path, image_output_dir_absolute_path, resolution_dpi);
        if (generated_pngs.length > 0) {
            core.info(`   - Generated ${generated_pngs.length} PNGs for ${link_file.path} at ref '${ref}'.`);
        }
        else {
            core.warning(`   - No PNGs generated from PDF for ${link_file.path} at ref '${ref}'.`);
        }
        ...
        await fs.promises
            ...
            .catch((rmErr) => core.warning(`   - Failed to remove temp PDF ${temp_pdf_path}: ${rmErr.message}`));
        results.push({
            link_file_path: link_file.path,
            base_name: link_file.base_name,
    ...
    await fs.promises
        ...
        .catch((rmErr) => core.warning(`Failed to remove PDF temp directory ${pdf_temp_dir}: ${rmErr.message}`));
    core.endGroup();
    return results;
}
/**
...
export async function generate_visual_diffs_for_pr(params) {
    ...
    if (params.gemini_api_key) {
        core.info(`Gemini Analysis Enabled: Yes (Model: ${params.gemini_model_name})`);
    }
    else {
    ...
    let repo_html_url;
    try {
        core.info(`Fetching base commit SHA and repo URL for PR #${params.pr_number}...`);
        ...
        if (!base_sha)
            throw new Error("Base SHA could not be determined.");
    }
    catch (error) {
        core.error(`Failed to get base SHA/repo URL for PR #${params.pr_number}: ${error.message}`);
        core.endGroup();
        throw error; // Cannot proceed without base SHA
    }
    // --- Find Changed Link Files in PR ---
    core.startGroup("Finding Changed Link Files in PR");
    const changed_link_files = []; // Use LinkFileInfo type
    const changed_file_statuses = {}; // Store status
    try {
        ...
        core.info(`Found ${changed_link_files.length} added/modified/renamed/removed link file(s) to process.`); // Update log
    }
    catch (error) {
    ...
    if (changed_link_files.length === 0) {
        core.info("No relevant changed link files found between base and head. Nothing to generate or analyze.");
        core.endGroup();
        return;
    }
    // --- Setup Temporary Directory for All Processing ---
    let processing_temp_dir = null;
    try {
        processing_temp_dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), `visual-diff-processing-${params.pr_number}-`));
        ...
        const files_for_head_generation = changed_link_files.filter((f) => changed_file_statuses[f.path] !== "removed");
        const files_for_base_generation = changed_link_files.filter((f) => changed_file_statuses[f.path] !== "added");
        // --- Generate Base Branch Images (Into Temp Dir) ---
        core.info(`Generating base images (ref: ${base_sha})...`);
        const base_image_results = await generate_images_for_ref(base_sha, files_for_base_generation, // Only generate for existing/modified/renamed/removed
        base_image_temp_dir, processing_temp_dir, params.octokit, params.drive, params.owner, params.repo, params.resolution_dpi);
        // --- Generate Head Branch Images (Into Temp Dir) ---
        core.info(`Generating head images (ref: ${params.head_sha})...`);
        const head_image_results = await generate_images_for_ref(params.head_sha, files_for_head_generation, // Only generate for existing/modified/renamed/added
        head_image_temp_dir, processing_temp_dir, params.octokit, params.drive, params.owner, params.repo, params.resolution_dpi);
        // --- Ensure Final Output Directory Exists ---
        core.info(`Ensuring final output directory exists: ${params.output_base_dir}`);
        ...
        for (const result of head_image_results) {
            if (result.png_paths.length > 0) {
                ...
                const image_output_dir_relative_path = path.join(relative_dir, result.base_name);
                const final_output_dir = path.join(params.output_base_dir, image_output_dir_relative_path);
                core.info(`Copying/Updating ${result.png_paths.length} PNGs for ${result.link_file_path} to ${final_output_dir}`);
                ...
                processed_files_info_for_commit.push(`'${result.link_file_path}' (${result.png_paths.length} pages)`);
            }
            else {
                core.info(`No head images generated for ${result.link_file_path}, nothing to copy.`);
            }
        }
        // Process removed files - delete their corresponding directories in the output
        const removed_files = changed_link_files.filter((f) => changed_file_statuses[f.path] === "removed");
        for (const removed_file of removed_files) {
            const relative_dir = path.dirname(removed_file.path);
            const image_output_dir_relative_path = path.join(relative_dir, removed_file.base_name);
            const final_output_dir_to_remove = path.join(params.output_base_dir, image_output_dir_relative_path);
            core.info(`Removing directory for deleted link file ${removed_file.path}: ${final_output_dir_to_remove}`);
            await fs.promises
                .rm(final_output_dir_to_remove, { recursive: true, force: true })
                .catch((rmErr) => core.warning(`Failed to remove directory ${final_output_dir_to_remove}: ${rmErr.message}`));
        ...
        if (total_pngs_generated_for_commit > 0 || removed_files.length > 0) {
            ...
            try {
                ...
                core.error("Visual diff PNG update succeeded, but committing/pushing changes failed.");
                throw commitError;
            }
        }
        else {
        ...
        if (params.gemini_api_key && changed_link_files.length > 0) {
            ...
            const base_results_map = new Map(base_image_results.map((r) => [r.link_file_path, r]));
            const head_results_map = new Map(head_image_results.map((r) => [r.link_file_path, r]));
            for (const link_file of changed_link_files) {
                ...
                core.info(`Processing file: ${link_file.path} (Status: ${file_status})`);
                if (file_status === "added") {
                    ...
                    if (head_result && head_result.png_paths.length > 0) {
                        for (let i = 0; i < head_result.png_paths.length; i++) {
                            ...
                            try {
                                const analysis = await analyze_image_diff_with_gemini(null, // No base image
                                head_png_path, params.gemini_api_key, params.gemini_model_name);
                                analysis_results.push({
                                    filePath: link_file.path,
                                    page: page_num,
                                    analysis,
                                ...
                                core.error(`   - Gemini analysis failed for added page ${page_num}: ${geminiError.message}`);
                                analysis_results.push({
                                    filePath: link_file.path,
                                    page: page_num,
                                    analysis: "*エラー: 追加されたページの分析中に問題が発生しました。*",
                                });
                            }
                        }
                    }
                    else {
                        core.warning(`   - No head images found for added file ${link_file.path}.`);
                    }
                }
                    ...
                    const analysis = await analyze_image_diff_with_gemini(base_result?.png_paths[0] ?? null, // Use first page of base if available for context (optional)
                    null, // No head image
                    params.gemini_api_key, params.gemini_model_name);
                    analysis_results.push({
                        filePath: link_file.path,
                        page: 0, // Use 0 for file-level status
                    ...
                    if (base_result && head_result) {
                        ...
                        core.info(`   - Analyzing modified/renamed file (Base: ${base_pages} pages, Head: ${head_pages} pages)`);
                        if (base_pages === 0 && head_pages > 0) {
                            analysis_results.push({
                                filePath: link_file.path,
                                page: 0,
                                analysis: "**コンテンツ追加** (ファイルは変更されましたが、以前は空でした)",
                            });
                            // Also analyze the new pages individually
                            for (let i = 0; i < head_pages; i++) {
                                const page_num = i + 1;
                                const head_png_path = head_result.png_paths[i];
                                core.info(`   - Analyzing added Page ${page_num} in modified file...`);
                                try {
                                    const analysis = await analyze_image_diff_with_gemini(null, head_png_path, params.gemini_api_key, params.gemini_model_name);
                                    analysis_results.push({
                                        filePath: link_file.path,
                                        page: page_num,
                                        analysis,
                            ...
                            analysis_results.push({
                                filePath: link_file.path,
                                page: 0,
                                analysis: "**コンテンツ削除** (ファイルは変更されましたが、現在は空です)",
                            });
                        }
                        else if (base_pages === 0 && head_pages === 0) {
                            core.warning(`   - Both base and head have no images for ${link_file.path}. Skipping.`);
                            continue;
                        }
                        ...
                        for (let i = 0; i < max_pages; i++) {
                            ...
                            if (base_png_path && head_png_path) {
                                ...
                                try {
                                    const analysis = await analyze_image_diff_with_gemini(base_png_path, head_png_path, params.gemini_api_key, // API key is checked at the start of the block
                                    ...
                                    core.error(`   - Gemini analysis failed for page ${page_num}: ${geminiError.message}`);
                                    analysis_results.push({
                                        filePath: link_file.path,
                                        page: page_num,
                        ...
                        core.warning(`Could not find both base and head image results for modified/renamed file: ${link_file.path}. Skipping analysis.`);
                    }
                }
            }
            // --- Post Comment to PR ---
            if (analysis_results.length > 0) {
                ...
                try {
                    ...
                    const comment_body = format_pr_comment(analysis_results, repo_html_url, // Pass repo URL for linking
                    ...
                    core.error(`Failed to post analysis comment to PR #${params.pr_number}: ${commentError.message}`);
            ...
            core.info("Skipping Gemini analysis as no relevant files were changed or processed.");
        }
        else {
        ...
        core.error(`Unhandled error during visual diff generation: ${error.message}`);
        core.setFailed(`Visual diff generation failed: ${error.message}`);
    }
    finally {
        // --- Cleanup Temp Directory ---
        if (processing_temp_dir) {
            core.info(`Cleaning up temporary processing directory: ${processing_temp_dir}`);
            await fs.promises
                .rm(processing_temp_dir, { recursive: true, force: true })
                .catch((rmErr) => core.warning(`Failed to remove processing temp directory ${processing_temp_dir}: ${rmErr.message}`));
        }
