...
const credentials_json = JSON.parse(
  Buffer.from(credentials_input, "base64").toString()
);
...
async function list_local_files(root_dir: string): Promise<FileInfo[]> {
  ...
  if (fs.existsSync(git_ignore_path)) {
    try {
      const gitignore_content = await fs_promises.readFile(
        git_ignore_path,
        "utf-8"
      );
      const gitignore_lines = gitignore_content
        .split("\n")
        .map((line) => line.trim())
      ...
      core.warning(
        `Could not read or parse .gitignore: ${(error as Error).message}`
      );
    }
  }
  ...
  for (const relative_path of all_files) {
    ...
    try {
      ...
      core.warning(
        `Could not stat file ${full_path}: ${(error as Error).message}`
      );
    }
  }
  ...
  try {
    ...
    do {
      const res = (await drive.permissions.list({
        fileId: file_id,
        fields:
          "nextPageToken, permissions(id, role, emailAddress, pendingOwner)",
    ...
    for (const perm of pending_permissions) {
      core.info(
        `Accepting ownership transfer for item ${file_id}, permission ID: ${perm.id}`
      );
    ...
    const file_meta = await drive.files.get({
      fileId: file_id,
      fields: "mimeType",
    });
    if (file_meta.data.mimeType === "application/vnd.google-apps.folder") {
      core.debug(
        `Checking children of folder ${file_id} for ownership transfers.`
      );
    ...
    if (err.code === 404 || err.code === 403) {
      core.debug(
        `Skipping ownership transfer check for item ${file_id} (may not exist or no permission): ${err.message}`
      );
    } else {
      core.warning(
        `Failed to process ownership transfers for item ${file_id}: ${err.message}`
      );
    }
  }
}
...
async function list_drive_files_recursively(
  ...
  core.info(
    `Listing items in Drive folder ID: ${folder_id} (relative path: '${
      base_path || "/"
    }')`
  );
  ...
  try {
    do {
      const res = (await drive.files.list({
        ...
        fields:
          "nextPageToken, files(id, name, mimeType, md5Checksum, owners(emailAddress), modifiedTime)",
        spaces: "drive",
        pageToken: next_page_token,
        pageSize: 1000,
      ...
      next_page_token = res.data.nextPageToken;
      core.debug(
        `Fetched page of items from folder ${folder_id}. Next page token: ${
          next_page_token ? "yes" : "no"
        }`
      );
    } while (next_page_token);
  } catch (error) {
    core.error(
      `Failed to list files in Drive folder ${folder_id}: ${
  ...
  core.info(
    `Processing ${all_items.length} items found in folder ID: ${folder_id}`
  );
  const service_account_email = credentials_json.client_email;
  ...
  for (const item of all_items) {
    if (!item.name || !item.id) {
      core.warning(
        `Skipping item with missing name or ID in folder ${folder_id}. Data: ${JSON.stringify(
          item
        )}`
      );
      continue;
    ...
    const relative_path = base_path
      ? path.join(base_path, item.name).replace(/\\/g, "/")
      : item.name.replace(/\\/g, "/");
    const owned =
      item.owners?.some(
        (owner) => owner.emailAddress === service_account_email
      ) || false;
    ...
    try {
      ...
      core.warning(
        `Could not list permissions for item ${item.id} ('${item.name}'): ${
          (permError as Error).message
        }`
      );
    }
    ...
    if (item.mimeType === "application/vnd.google-apps.folder") {
      ...
      try {
        const subfolder_data = await list_drive_files_recursively(
          item.id,
          relative_path
        );
        ...
        core.error(
          `Error processing subfolder ${item.id} ('${item.name}'): ${
      ...
      file_map.set(relative_path, {
        ...
        owned,
        permissions,
        lastModified: (item as any).modifiedTime,
        isGoogleDoc: item.mimeType === "application/vnd.google-apps.document",
      });
    }
  }
...
// Ensure Folder
async function ensure_folder(
  parent_id: string,
  folder_name: string
): Promise<string> {
  core.info(`Ensuring folder '${folder_name}' under parent '${parent_id}'`);
  try {
    const query = `'${parent_id}' in parents and mimeType = 'application/vnd.google-apps.folder' and name = '${folder_name.replace(
      /'/g,
      "\\'"
    )}' and trashed = false`;
    ...
    core.debug(
      `API response for existing folder query '${folder_name}' under '${parent_id}': ${JSON.stringify(
        res.data
      )}`
    );
    ...
    if (existing_folder && existing_folder.id) {
      core.info(
        `Reusing existing folder '${folder_name}' with ID: ${existing_folder.id}`
      );
    ...
    if (!folder.data.id) {
      throw new Error(
        `Folder creation API call did not return an ID for '${folder_name}'.`
      );
    }
    ...
    const err = error as any;
    core.error(
      `Failed to ensure folder '${folder_name}' under '${parent_id}': ${err.message}`
    );
...
async function build_folder_structure(
  ...
  for (const file of local_files) {
    ...
    if (dir && dir !== ".") {
      ...
      for (const part of parts) {
        current_cumulative_path = current_cumulative_path
          ? path.join(current_cumulative_path, part)
          : part;
  ...
  core.info(
    `Required folder paths based on local files: ${
      sorted_paths.join(", ") || "None"
    }`
  );
  ...
  for (const folder_path of sorted_paths) {
    ...
    if (!parent_folder_id) {
      core.error(
        `Cannot find parent folder ID for path '${folder_path}' (parent path '${parent_path}' missing from map). Skipping.`
      );
      continue;
    }
    ...
    if (existing_drive_folder?.id) {
      core.info(
        `Using existing Drive folder '${folder_path}' with ID: ${existing_drive_folder.id}`
      );
      current_folder_id = existing_drive_folder.id;
    } else {
      core.info(
        `Creating missing folder '${folder_name}' under parent ID ${parent_folder_id} (for path '${folder_path}')`
      );
      try {
        current_folder_id = await ensure_folder(parent_folder_id, folder_name);
      } catch (error) {
        core.error(
          `Failed to create folder structure at '${folder_path}'. Stopping structure build.`
        );
        throw error;
      }
    }
  ...
  core.info(
    `Built/Verified folder structure. Path-to-ID map size: ${folder_map.size}`
  );
  return folder_map;
}
...
async function upload_file(
  file_path: string,
  folder_id: string,
  existing_file?: { id: string; name: string }
  ...
  try {
    if (existing_file?.id) {
      ...
      if (existing_file.name !== file_name) {
        ...
        core.info(
          `Updating file name for '${existing_file.name}' to '${file_name}' (ID: ${existing_file.id})`
        );
      }
      core.info(
        `Updating existing file content '${file_name}' (ID: ${existing_file.id}) in folder ${folder_id}`
      );
      ...
      fileId = res.data.id!;
      core.info(
        `Updated file '${res.data.name}' (ID: ${fileId}). New hash: ${
          res.data.md5Checksum || "N/A"
        }`
      );
    } else {
      ...
      if (!res.data.id) {
        throw new Error(
          `File creation API call did not return an ID for '${file_name}'.`
        );
      }
      fileId = res.data.id;
      core.info(
        `Uploaded file '${res.data.name}' (ID: ${fileId}). Hash: ${
          res.data.md5Checksum || "N/A"
        }`
      );
    }
    ...
    const err = error as any;
    core.warning(
      `Failed to process '${file_name}' in folder ${folder_id}: ${err.message}`
    );
    if (err.response?.data) {
...
// Delete Untracked
async function delete_untracked(
  id: string,
  name: string,
  ...
  try {
    ...
    core.info(
      `Moved untracked ${is_folder ? "folder" : "file"} to Trash: ${name}`
    );
    return true;
  } catch (error: unknown) {
    const err = error as any;
    if (err.code === 403) {
      core.error(
        `Permission denied trying to trash ${
          is_folder ? "folder" : "file"
        } '${name}' (ID: ${id}). Check service account permissions.`
      );
    } else if (err.code === 404) {
      core.warning(
        `Untracked ${
          is_folder ? "folder" : "file"
        } '${name}' (ID: ${id}) not found, possibly already deleted.`
      );
      return true;
    } else {
      core.warning(
        `Failed to trash untracked ${
          is_folder ? "folder" : "file"
        } '${name}' (ID: ${id}): ${err.message}`
      );
    }
    if (err.response?.data) {
      core.warning(`API Error Details: ${JSON.stringify(err.response.data)}`);
...
async function request_ownership_transfer(
  file_id: string,
  ...
  try {
    const service_account_email = credentials_json.client_email;
    core.info(
      `Requesting ownership transfer of item ${file_id} from ${current_owner_email} to ${service_account_email}`
    );
    ...
    core.info(
      `Ownership transfer request sent for item ${file_id}. Response: ${JSON.stringify(
        response.data
      )}`
    );
    ...
    if (err.message && err.message.includes("Consent is required")) {
      core.error(
        `Failed to request ownership transfer for item ${file_id}: Owner (${current_owner_email}) must grant consent or domain admin needs to configure settings.`
      );
    } else if (err.code === 403) {
      core.error(
        `Permission denied requesting ownership transfer for item ${file_id}. Check service account permissions and Drive sharing settings.`
      );
    } else {
      core.warning(
        `Failed to request ownership transfer for item ${file_id}: ${err.message}`
      );
    }
    if (err.response?.data) {
      core.warning(`API Error Details: ${JSON.stringify(err.response.data)}`);
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      res.data
        ...
        .on("error", (err) => {
          core.error(
            `Error writing downloaded file ${file_id} to ${local_path}: ${err.message}`
          );
          fs.unlink(local_path, (unlinkErr) => {
            if (unlinkErr)
              core.warning(
                `Failed to clean up partial download ${local_path}: ${unlinkErr.message}`
              );
            reject(err);
          });
        });
    ...
    if (err.code === 404) {
      core.error(
        `Failed to download file ${file_id}: File not found in Google Drive.`
      );
    } else if (err.code === 403) {
      core.error(
        `Failed to download file ${file_id}: Permission denied. Check service account access.`
      );
    } else {
...
async function execGit(
  ...
  try {
    ...
    if (!options.ignoreReturnCode && result.exitCode !== 0) {
      core.error(
        `Git command failed: git ${command} ${args.join(" ")} - Exit Code: ${
          result.exitCode
        }`
      );
    ...
    core.debug(
      `Git command finished: git ${command} - Exit Code: ${result.exitCode}`
    );
    core.debug(`stdout: ${result.stdout}`);
    if (result.stderr && !(options.silent && result.exitCode === 0)) {
      // Avoid logging stderr on success if silent
  ...
  for (let attempt = 0; attempt < max_retries; attempt++) {
    try {
      ...
      core.info(
        `Attempt ${
          attempt + 1
        }: Creating PR: head=${head_ref} base=${base_branch}`
      );
      ...
      core.info(
        `Pull request created successfully! (head: ${head_ref}, base: ${base_branch})`
      );
      return;
    } catch (error: unknown) {
      ...
      if (http_error?.response?.data)
        core.warning(
          `API Response Data: ${JSON.stringify(http_error.response.data)}`
        );

      if (
        (http_error?.status === 404 || http_error?.status === 422) &&
        ...
        core.error(
          `Failed to create pull request after ${attempt + 1} attempts.`
        );
        throw error;
      }
    }
...
// Google Docsをエクスポートする関数
async function export_google_doc(doc_id: string): Promise<string> {
  try {
    ...
    core.error(
      `Failed to export Google Doc ${doc_id}: ${(error as Error).message}`
    );
    throw error;
  }
}
...
async function check_google_docs_changes(
  drive_files: Map<string, DriveItem>
): Promise<{
  new_docs: GoogleDocsExport[];
  modified_docs: GoogleDocsExport[];
}> {
  const new_docs: GoogleDocsExport[] = [];
  const modified_docs: GoogleDocsExport[] = [];

  for (const [path, item] of drive_files) {
      ...
      try {
        ...
        if (fs.existsSync(html_path)) {
          ...
          if (local_hash !== html_hash) {
            modified_docs.push(doc_export);
          }
        } else {
          new_docs.push(doc_export);
        }
      } catch (error) {
        core.warning(
          `Failed to process Google Doc ${item.id} at ${path}: ${
            (error as Error).message
          }`
        );
      }
...
async function handle_drive_changes(
  ...
  core.info(
    `Handling potential incoming changes from Drive folder: ${folder_id} (Trigger: ${trigger_event_name}, Untrack action: ${on_untrack_action})`
  );
  ...
  const current_branch_result = await execGit(
    "rev-parse",
    ["--abbrev-ref", "HEAD"],
    { silent: true }
  );
  ...
  core.info(
    `Current branch is '${initial_branch}'. Creating temporary state branch '${original_state_branch}'`
  );
  const initial_commit_hash = (
    await execGit("rev-parse", ["HEAD"], { silent: true })
  ).stdout.trim();
  ...
  const local_map = new Map(
    local_files_list.map((f) => [f.relative_path.replace(/\\/g, "/"), f])
  );
  core.info(`Found ${local_map.size} relevant local files in original state.`);
  const local_lower_to_original_key = new Map(
    Array.from(local_map.keys()).map((key) => [key.toLowerCase(), key])
  );
  core.debug(
    `Created lowercase lookup map with ${local_lower_to_original_key.size} entries.`
  );
  ...
  try {
    ...
    drive_files = new Map(
      Array.from(drive_data.files.entries()).map(([p, item]) => [
        p.replace(/\\/g, "/"),
        item,
      ])
    );
    ...
    core.info(
      `Found ${drive_files.size} files and ${drive_folders.size} folders in Drive.`
    );
  } catch (error) {
    core.error(
      `Failed list Drive content for folder ${folder_id} during incoming check: ${
        (error as Error).message
      }. Aborting.`
    );
  ...
  for (const [drive_path, drive_item] of drive_files) {
    ...
    const original_local_key =
      local_lower_to_original_key.get(drive_path_lower);
    const local_file = original_local_key
      ? local_map.get(original_local_key)
      : undefined;
    core.debug(
      `Comparing Drive path: '${drive_path}', Lowercase: '${drive_path_lower}'`
    );
    if (!local_file || !original_local_key) {
      core.debug(`   Local file NOT FOUND for Drive path: '${drive_path}'`);
      core.info(
        `New file detected in Drive: ${drive_path} (ID: ${drive_item.id})`
      );
      new_files.push({ path: drive_path, id: drive_item.id });
    } else {
      core.debug(
        `   Found matching local key (case-insensitive): '${original_local_key}'`
      );
      found_local_keys.add(original_local_key);
      if (drive_item.hash && local_file.hash !== drive_item.hash) {
        core.info(
          `Modified file detected in Drive (hash mismatch): ${drive_path}`
        );
        core.info(
          ` -> Local hash: ${local_file.hash}, Drive hash: ${drive_item.hash}`
        );
        modified_files.push({
          path: drive_path,
          id: drive_item.id,
          local_hash: local_file.hash,
          drive_hash: drive_item.hash,
        });
      } else if (
        !drive_item.hash &&
        drive_item.mimeType &&
        !drive_item.mimeType.startsWith("application/vnd.google-apps")
      ) {
        core.warning(
          `Drive file ${drive_path} exists locally but has no md5Checksum. Cannot verify modification. Skipping update from Drive.`
        );
      } else {
        core.debug(
          `File '${drive_path}' found locally and hashes match or is Google Doc. No modification needed.`
        );
      }
    }
  }
  ...
  for (const [local_key, _local_file_info] of local_map) {
    if (!found_local_keys.has(local_key)) {
      core.info(
        `File deleted in Drive detected (was in local state, not found in Drive): ${local_key}`
      );
      deleted_files.push(local_key);
    }
  }
  core.debug(
    `Identified ${deleted_files.length} files potentially deleted in Drive.`
  );
  ...
  // --- Handle Google Docs Changes ---
  core.info("Checking for Google Docs changes...");
  const { new_docs, modified_docs } = await check_google_docs_changes(
    drive_files
  );
  ...
  // --- Handle New Files ---
  core.debug(
    `Applying changes: ${new_files.length} new, ${modified_files.length} modified, ${deleted_files.length} potentially deleted.`
  );
  for (const { path: file_path, id } of new_files) {
    try {
      ...
      core.error(
        `Failed to download or stage new file ${file_path}: ${
          (error as Error).message
        }`
      );
    }
  ...
  for (const { path: file_path, id } of modified_files) {
    try {
      core.info(
        `Downloading modified file from Drive: ${file_path} (ID: ${id})`
      );
      await download_file(id, file_path);
      await execGit("add", [file_path]);
      changes_staged = true;
      ...
      core.error(
        `Failed to download or stage modified file ${file_path}: ${
          (error as Error).message
        }`
      );
    }
  ...
  if (trigger_event_name === "push") {
    ...
    if (deleted_files.length > 0) {
      core.info(
        `Found ${deleted_files.length} file(s) present locally but not in Drive. Skipping removal from Git because trigger was 'push'.`
      );
      deleted_files.forEach((fp) =>
        core.info(`  - Skipped removal (push event): ${fp}`)
      );
    }
  } else {
    // trigger_event_name is 'schedule', 'workflow_dispatch', etc.
    // For schedule/manual triggers, respect the on_untrack setting
    if (on_untrack_action === "remove") {
      core.info(
        `Processing ${deleted_files.length} files potentially deleted in Drive (trigger: ${trigger_event_name}, on_untrack: 'remove').`
      );
      for (const file_path of deleted_files) {
        try {
          ...
          core.error(
            `Failed to stage deletion of ${file_path}: ${
              (error as Error).message
            }`
          );
        }
      ...
      if (deleted_files.length > 0) {
        core.info(
          `Found ${deleted_files.length} file(s) present locally but not in Drive. Skipping removal from Git because 'on_untrack' is '${on_untrack_action}' (trigger: ${trigger_event_name}).`
        );
        deleted_files.forEach((fp) =>
          core.info(`  - Skipped removal (config): ${fp}`)
        );
      }
    }
  }
  ...
  if (changes_staged) {
    ...
    if (!status_result.stdout.trim()) {
      core.info(
        "Git status clean after applying changes. No commit needed for incoming changes."
      );
  ...
  if (changes_staged) {
    core.info(
      "Changes detected originating from Drive. Proceeding with commit and PR."
    );
    const commit_messages: string[] = ["Sync changes from Google Drive"];
    if (new_files.length > 0)
      commit_messages.push(
        `- Added: ${new_files.map((f) => f.path).join(", ")}`
      );
    if (modified_files.length > 0)
      commit_messages.push(
        `- Updated: ${modified_files.map((f) => f.path).join(", ")}`
      );
    if (files_actually_removed.length > 0)
      commit_messages.push(`- Removed: ${files_actually_removed.join(", ")}`);
    ...
    try {
      ...
      const pr_body =
        ...
        (modified_files.length > 0
          ? `*   **Updated:** ${modified_files
              ...
              .join(", ")}\n`
          : "") +
        (files_actually_removed.length > 0
          ? `*   **Removed:** ${files_actually_removed
              .map((f) => `\`${f}\``)
              .join(", ")}\n`
      ...
      core.setFailed(
        `Failed during commit, push, or PR creation for Drive changes: ${
          (error as Error).message
        }`
      );
    }
    ...
    core.info(
      "No effective changes detected originating from Drive to ADD, UPDATE, or REMOVE (based on config/trigger). No PR needed."
    );
  }

  // *** 7. Cleanup: Go back to the original branch and delete the temporary state branch ***
  core.info(
    `Cleaning up temporary branches. Checking out initial branch '${initial_branch}'`
  );
  try {
    await execGit("checkout", [initial_branch]);
    ...
    core.warning(
      `Failed to checkout initial branch '${initial_branch}' or delete temp branch '${original_state_branch}'. Manual cleanup may be needed. Error: ${
    ...
    try {
      ...
      core.error(
        "Could not checkout 'main' either. Workspace might be in an inconsistent state."
      );
    }
  }
}
...
async function sync_to_drive() {
  ...
  if (initial_local_files.length === 0) {
    core.warning(
      "No relevant local files found to sync based on ignore rules."
    );
  } else {
    core.info(
      `Found ${initial_local_files.length} initial local files for potential sync.`
    );
  }
  ...
  for (const target of config.targets.forks) {
    ...
    try {
      ...
      const current_local_map = new Map(
        current_local_files.map((f) => [f.relative_path.replace(/\\/g, "/"), f])
      );
      core.info(
      ...
      try {
        ...
        const drive_data_outgoing = await list_drive_files_recursively(
          folder_id
        );
        drive_files_map_outgoing = new Map(
          Array.from(drive_data_outgoing.files.entries()).map(([p, item]) => [
            p.replace(/\\/g, "/"),
            item,
          ])
        );
        drive_folders_map_outgoing = new Map(
          Array.from(drive_data_outgoing.folders.entries()).map(([p, item]) => [
            p.replace(/\\/g, "/"),
            item,
          ])
        );
        core.info(
          `Found ${drive_files_map_outgoing.size} files and ${drive_folders_map_outgoing.size} folders in Drive.`
        );
      } catch (listError) {
        /* Handle error */
        core.error(
          `Failed list Drive content before outgoing sync: ${
            (listError as Error).message
          }. Skipping outgoing sync.`
        );
        drive_files_map_outgoing = new Map();
        drive_folders_map_outgoing = new Map();
      }
      ...
      if (current_local_map.size > 0 && drive_folders_map_outgoing) {
        /* Build structure */
        try {
          folder_path_to_id_map = await build_folder_structure(
            folder_id,
            current_local_files,
            drive_folders_map_outgoing
          );
        } catch (structureError) {
          core.error(
            `Failed build Drive folder structure: ${
              (structureError as Error).message
            }. Skipping upload/update.`
          );
          folder_path_to_id_map = new Map();
        }
      } else {
        /* Handle no local files or failed list */
        folder_path_to_id_map = new Map();
        folder_path_to_id_map.set("", folder_id);
        core.info(
          "Skipping folder structure build (no local files or Drive folder listing failed)."
        );
      }
      core.info("Uploading/updating files from local to Drive...");
      const files_synced_to_drive = new Set<string>();
      if (folder_path_to_id_map.size > 0) {
        /* Upload/Update loop */
        for (const [relative_path, local_file] of current_local_map) {
          ...
          if (!target_folder_id) {
            core.warning(
              `Could not find target folder ID for local file '${relative_path}'. Skipping upload.`
            );
            continue;
          }
          // --- Upload/Update Logic ---
          if (!drive_file) {
            ...
            if (drive_file.hash && drive_file.hash !== local_file.hash) {
              ...
              !drive_file.hash &&
              drive_file.mimeType &&
              !drive_file.mimeType.startsWith("application/vnd.google-apps")
            ) {
              await upload_file(local_file.path, target_folder_id, {
                id: drive_file.id,
                name: file_name,
            ...
            const hashes_match_or_drive_missing =
              !drive_file.hash || local_file.hash === drive_file.hash;
            if (
              drive_file.name !== file_name &&
              hashes_match_or_drive_missing
            ) {
              try {
                await drive.files.update({
                ...
                core.warning(
                  `Failed rename file ${drive_file.id}: ${
        ...
        core.info(
          "Skipping upload/update phase due to issues in previous steps."
        );
      }
      ...
      const untracked_drive_files = Array.from(
        drive_files_map_outgoing.entries()
      ).filter(([p]) => !files_synced_to_drive.has(p));
      const untracked_drive_folders = Array.from(
        drive_folders_map_outgoing.entries()
      ).filter(
        ([p]) =>
          !Array.from(files_synced_to_drive).some((lp) =>
            lp.startsWith(p + "/")
          )
      );
      core.info(
        `Found ${untracked_drive_files.length} untracked files and ${untracked_drive_folders.length} potentially untracked folders.`
      );
      const all_untracked_items = [
        /* Combine files and folders */ ...untracked_drive_files.map(
          ([p, i]) => ({ path: p, item: i, isFolder: false })
        ),
        ...
        if (on_untrack_action === "ignore") {
          ...
          for (const {
            ...
            core.info(
              ...
              } in Drive: ${untracked_path} (ID: ${untracked_item.id})`
            );
            if (!untracked_item.owned) {
              /* Handle not owned */
              const current_owner = untracked_item.permissions?.find(
                (p) => p.role === "owner"
              )?.emailAddress;
              if (
                current_owner &&
                current_owner !== credentials_json.client_email
              ) {
                ...
                /* Unclear owner */
                const ownerEmails =
                  untracked_item.permissions
                    ?.filter((p) => p.role === "owner")
                    .map((p) => p.emailAddress || "Unknown")
                    .join(",") || "None";
                core.warning(
                  `Untracked '${untracked_path}' has unclear ownership (Owners: ${ownerEmails}).`
                );
                if (on_untrack_action === "remove") {
                  core.warning(
                    `Skipping removal '${untracked_path}' due to unclear ownership.`
                  );
                  continue;
                }
              }
              ...
              if (on_untrack_action === "remove") {
                ...
                core.info(
                  `Untracked '${untracked_path}' already owned. No action needed.`
                );
              }
            }
          }
      ...
      core.info(
        "Step 3: Checking for and accepting pending ownership transfers..."
      );
