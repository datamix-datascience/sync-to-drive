...
async function list_drive_files(
  folder_id: string,
...
async function sync_to_drive() {
  ...
  if (local_files.length === 0) {
    core.setFailed(
      "No files found in repository to sync (after applying ignore patterns)",
    );
    return;
  }
  ...
  for (const target of config.targets.forks) {
    ...
    if (drive_files.size > 0) {
      if (target.on_untrack === "remove") {
        ...
        core.info(
          `Leaving ${drive_files.size} untracked files in folder ${folder_id}`,
        );
      }
    }
